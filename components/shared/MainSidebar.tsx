...
import { ResactLogo } from './ResactLogo';
                ...
                {isExpanded && (
                  <div className="ml-6 space-y-1">
                    {categoryComponents.map((component) => {
                      ...
                      const isDisabled = component.name === 'Auth Card';
                      
                      return (
                        <button
                          key={component.name}
                          onClick={isDisabled ? undefined : () => onComponentClick(component.name)}
                          disabled={isDisabled}
                          ...
                          title={isDisabled ? 'This component is currently unavailable' : undefined}
                        >
                          <ComponentIcon className={`w-3.5 h-3.5 flex-shrink-0 ${isDisabled ? 'opacity-50' : ''}`} />
                          <span>{component.name}</span>
                          {isDisabled && (
                            <span className="ml-auto text-xs opacity-60">(Disabled)</span>
                          )}
                        </button>
                      );
                    })}
