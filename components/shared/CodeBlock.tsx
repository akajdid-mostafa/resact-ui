...
export const CodeBlock: React.FC<CodeBlockProps> = ({ 
  ...
  const { lines, shouldTruncate, truncatedCode, displayCode } = useMemo(() => {
  ...
  return (
    <div className="relative group">
      {title && (
        <div className="text-sm font-medium mb-2" style={{ color: 'var(--color-text-secondary)' }}>{title}</div>
      )}
            ...
            <div 
              ...
              style={{
                maxHeight: shouldTruncate 
                  ? (isExpanded ? contentHeight ? `${contentHeight}px` : 'none' : CODE_BLOCK_ANIMATIONS.TRUNCATED_HEIGHT)
                  : 'none',
                transitionDuration: `${CODE_BLOCK_ANIMATIONS.EXPAND_DURATION}ms`
              }}
            >
              <pre className="bg-white p-4 rounded-md overflow-x-auto font-mono" style={{ 
              ...
              <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 z-10">
                <Button
                  ...
                  className={`h-7 px-3 bg-white hover:bg-brown-50 border border-brown-200 rounded-full shadow-sm transition-all ${
                    isAnimating 
                      ? 'scale-95 opacity-80' 
                      : 'scale-100 opacity-100 hover:shadow-md'
                  }`}
                  style={{ 
                    color: 'var(--color-text-tertiary)',
                    transitionDuration: `${CODE_BLOCK_ANIMATIONS.EXPAND_DURATION}ms`,
                    transform: isAnimating 
                      ? `scale(${CODE_BLOCK_ANIMATIONS.BUTTON_SCALE_ACTIVE})` 
                      : `scale(1)`,
                  }}
                  onMouseEnter={(e) => {
                    if (!isAnimating) {
